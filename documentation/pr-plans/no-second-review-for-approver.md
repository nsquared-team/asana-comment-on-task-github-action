...
- [x] A test for each stale-list case that fails on `main` and passes with the fix; a test for the genuine re-request that fails with the timeline check removed; every existing test still passes.
